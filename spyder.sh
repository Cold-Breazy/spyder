...
msg_exit() {
	{ clear; banner; echo; }
	echo -e "${GREENBG}${BLACK} I Hope You Enjoyed Using This Tool. Bye Bye.=""${RESETBG}\n"
	{ reset_color; exit 0; }
}
...
about() {
	...
	read -p "${RED}[${WHITE}=${RED}]${GREEN} Choose Option => ${BLUE}"
...
main_menu() {
	{ clear; banner; echo; }
	cat <<- EOF
             ${RED}[${WHITE}=>${RED}]${ORANGE} BREAZY CODING'S ${RED}[${WHITE}<=${RED}]${ORANGE}
